...
> ```
> 
> 该工具仍可优化，例如参考原有的签到工具，添加对扫码签到的监控(即 `isNumber` 和 `isRadar` 的值都为 `False`), 然后利用 Server酱 或其他工具发送链接，同样能实现远程挂机的效果，但本仓库将不再维护。
...
- 程序会生成一个 180 秒有效的 **临时链接**，将该链接发送给 **在教室里的同学**，链接打开会请求使用摄像头，让他们扫描二维码即可。
